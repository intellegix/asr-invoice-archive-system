...
class ProductionSettings(BaseSettings):
    ...
    VERSION: str = "2.0.0"
    DESCRIPTION: str = "Enterprise document processing server with sophisticated capabilities"
    ...
    def storage_config(self) -> Dict[str, Any]:
        ...
        config = {
            ...
            "base_path": str(self.get_data_dir)
        }
        ...
        if self.STORAGE_BACKEND == "s3":
            ...
            config.update({
                "mount_path": self.RENDER_DISK_MOUNT
            })
    ...
    def validate_production_configuration(self) -> 'ProductionSettings':
        ...
        if (self.AWS_DEPLOYMENT_MODE or self.RENDER_DEPLOYMENT_MODE) and self.DATABASE_URL.startswith("sqlite:"):
            raise ValueError("SQLite database not supported in cloud deployment. Use PostgreSQL.")
        ...
        if self.MULTI_TENANT_ENABLED and not self.TENANT_ISOLATION_ENABLED:
            raise ValueError("TENANT_ISOLATION_ENABLED must be true when multi-tenant is enabled")

        # Validate scanner API configuration
        if self.SCANNER_API_ENABLED and not self.API_KEYS_REQUIRED:
            raise ValueError("API_KEYS_REQUIRED must be true when scanner API is enabled")

        # Validate sophisticated processing configuration
        if len(self.PAYMENT_DETECTION_METHODS) < 3:
            raise ValueError("At least 3 payment detection methods required for consensus")
